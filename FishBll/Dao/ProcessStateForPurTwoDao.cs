...
{
    public class ProcessStateForPurTwoDao
    {
        /// <summary>
        /// 获取数据列表
        /// </summary>
        ...
        public DataTable getTableView ( string strWhere )
        {
            ...
            //strSql . Append ( "select a.Numbering,b.product_id,a.code,a.demand,a.Signdate,a.createman,c.code code1,i.createman createman2,j.code code2,case when e.coun2=0 then 0 else 0+convert(convert(e.coun3/e.coun2,decimal(11,4))*100,char) end coun1,case when f.coun2=0 then 0 else 0+convert(convert(f.coun3/f.coun2,decimal(11,4))*100,char) end coun2,case when g.coun2=0 then 0 else 0+convert(convert(g.coun3/g.coun2,decimal(11,4))*100,char) end coun3,case when h.coun2=0 then 0 else 0+convert(convert(h.coun3/h.coun2,decimal(11,4))*100,char) end coun4,0+convert(l.ton,char) ton1,0+convert(m.ton,char) ton2 " );
            ...
            //strSql . Append ( ") h on h.Numbering=a.Numbering and h.FishMealId=b.product_id " );
            ...
            //strSql . Append ( "left join (select b.NumberingOne,b.product_id,case when sum(b.Quantity)=0 then 0 else convert(sum(a.weight)/sum(b.Quantity),decimal(11,2))*100 end ton from t_OutboundOrder a right join t_happening b on a.codeNumContract=b.NumberingOne and a.FishMealId=b.product_id group by b.NumberingOne,b.product_id) m on m.NumberingOne=i.Numbering and m.product_id=b.product_id " );

            strSql.Append(" select a.Numbering,a.overflow,b.product_id,i.ContractCode,a.demand,a.Signdate,o.realname createman, c.realname code1, p.realname createman2, j.realname code2, b.Quantity,m.weight,m.Numbering,case when d.coun1 = 0 then 0 else 0 + convert(convert(d.coun / d.coun1, decimal(11, 2)) * 100, char) end coun, n.ConfirmTheWeight,n.Numbering Numbering4,case when e.coun2 = 0 then 0 else 0 + convert(convert(e.coun3 / e.coun2, decimal(11, 2)) * 100, char) end coun1,0.Chargeback,o.Numbering Numbering5,case when f.coun2 = 0 then 0 else 0 + convert(convert(f.coun4 / f.coun2, decimal(11, 2)) * 100, char) end coun2, g.code code3, p.Numbering Numbering6, p.rmb,n.ConfirmTheWeight* b.unitprice unitCon, case when h.coun2 = 0 then 0 else 0 + convert(convert(h.coun5 / h.coun2, decimal(11, 2)) * 100, char) end coun4, k.codeNum Numbering2, k.ton,l.Numbering Numbering3, ton1 ");
            ...
            // 出库单总重量
            strSql.Append(" left join(select convert(sum(weight), decimal(11, 2)) ton, FishMealId, codeNum from t_outboundorder group by FishMealId, codeNum) k on k.codeNum = a.Numbering and k.FishMealId = b.product_id ");
            // 磅单总净重
...
left join (select b.NumberingOne,b.product_id,case when sum(b.Quantity)=0 then 0 else convert(sum(a.weight)/sum(b.Quantity),decimal(11,2))*100 end ton 
from t_OutboundOrder a right join t_happening b on a.codeNumContract=b.NumberingOne and a.FishMealId=b.product_id
group by b.NumberingOne,b.product_id) m on m.NumberingOne=i.Numbering and m.product_id=b.product_id  */
