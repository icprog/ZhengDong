...
namespace UILibrary
{
    ...
    /// 下载进度显示控件
    /// </summary>
    public partial class DownloadSplashControl : UserControl
    {
        public event CancelEventHandler CancelEvent = null;

        public DownloadSplashControl()
        {
            InitializeComponent();
        ...
        public void ShowMessage(string msg, int precent)
        {
            ...
            if (precent <= progressBar1.Minimum)
            {
                progressBar1.Value = progressBar1.Minimum;
            }
            else if (precent >= progressBar1.Maximum)
            {
        ...
        private void DownloadSplashControl_SizeChanged(object sender, EventArgs e)
        {
            ChangeLocation();
        }
